...
export class MessageService {
  ...
  getAllMessage() {
    // this.http.get<Message[]>('assets/api/messages.json').pipe(tap(x => x)).subscribe(ms => {
    //   this.messages = ms;
    // });
    this.messages = this.data as Message[];
  }

  getMessageByFolder(folder: string) {
    const username = localStorage.getItem('UsernameLogin');
    return this.messages.filter(x => x.to === username && x.folder === folder);
  }

  getMessageById(id: string) {
    return this.messages.find(x => x._id === id);
  }
}
